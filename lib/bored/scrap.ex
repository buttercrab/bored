defmodule Bored.Scrap do
  @spec prob_info(prob_id :: integer()) :: %{id: integer(), title: String.t(), is_eng: true | false, tier: String.t()}
                                           | {:error, HTTPoison.Error.t()} | {:error, String.t()} | :error
  def prob_info(prob_id) do
    with {:ok, html} <- Bored.Baekjoon.get("/problem/#{prob_id}"),
         ...
         is_eng <- desc
                   ...
                   |> (&(&1 * 2 > length(desc))).(),
         {:ok, html} <- Bored.SolvedAc.get("/search?query=id%3A%20#{prob_id}"),
         {:ok, doc} <- Floki.parse_document(html.body),
         :ok <- Bored.SolvedAc.check_404(doc),
         tier <- doc
                 |> Floki.find("img")
                 |> List.last()
                 |> Floki.attribute("alt") do
      %{id: prob_id, title: title, is_eng: is_eng, tier: tier}
    end
  end

  @spec user_info(user_id :: String.t()) :: %{id: String.t(), solved: [integer()], tier: String.t()}
                                            | {:error, HTTPoison.Error.t()} | {:error, String.t()} | :error
  ...
  @spec rand_prob(tier :: {String.t(), String.t()}) :: [integer()]
                                                       | {:error, HTTPoison.Error.t()} | {:error, String.t()} | :error
